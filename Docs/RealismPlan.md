...
| **D1·D2·D4** | 바다 굴절 · 하늘 반사 · 스넬의 창 | 0.2.62 | ⏳ 코드 완료, **컴파일 검증 전** |
| **E1** | 전역 젖음 확대(구조물·바위·통나무·지면) | 0.2.62 | ⏳ 코드 완료, **컴파일 검증 전** |
| **F3** | 크리처 밟힘 | 0.2.62 | ⏳ 코드 완료, **컴파일 검증 전** |
| A1·A2 | 포스트프로세싱 볼륨 일원화 · 죽은 데이터 정리 | — | 미착수(화면을 보며 해야 한다) |
| E2 | 웅덩이 | — | 미착수 |
| **—** | 야자수 흔들림(FoliageSway) | 0.2.62 | ⏳ 코드 완료, **컴파일 검증 전** |

### 0.2.62에서 검수가 잡아낸 것 (컴파일러 없이 만든 두 번째 파도)

적대적 정적 검수 1회로 여섯 건이 걸렸고 전부 반영했다. 그중 셋은 **기능이 목적을 못 이루는** 종류였다.

- **하늘 반사가 정지된 하늘을 비출 뻔했다.** 스카이박스 머티리얼의 프로퍼티만 바꾸는 것으로는
  유니티의 기본 리플렉션 큐브맵이 갱신되지 않는다. 그대로 뒀으면 수면이 **씬 로드 순간의 하늘**을
  영원히 되비쳐, "노을이 져도 수면은 파랗다"는 고치려던 문제가 그대로 남았을 것이다.
  DayNightCycle이 태양각 2도마다 `DynamicGI.UpdateEnvironment()`를 부르도록 했다.
  (예전에 이 호출을 피한 이유는 "밤 스카이박스에서 뽑은 환경광이 0이라 밤이 안 보인다"였는데,
  지금은 환경광을 Trilight로 코드가 직접 정하므로 그 이유가 성립하지 않는다.)
- **건축 미리보기 고스트가 비 오면 불투명해질 뻔했다.** 젖음이 등록 시점의 알파를 되쓰는데,
  고스트는 등록 **직후에** 알파를 0.38로 낮춘다. 알파는 손대지 않도록 고쳤다.
- **마르면 원래 값으로 돌아온다는 계약이 깨져 있었다.** 임계값(0.05) 때문에 젖음이 정확히 0이 돼도
  마지막 적용값이 0.04쯤에 남아 세계가 영영 살짝 어둡게 굳는다. 끝점은 임계와 무관하게 적용한다.
- 굴절 합성이 중간값에서 하드웨어 블렌딩과 최대 6% 어긋나 수심을 따라 색 띠가 생길 뻔했다.
  앞면에서는 항상 완전 수동 합성하도록 바꿔 오차를 0으로 만들었다.
- 젖음 명부가 O(n²) 등록 + 소나기 시작 시 3프레임마다 0.5ms 히치. HashSet 등록 + 임계 완화 +
  저빈도 프루닝으로 고쳤다.
- 곰이 절벽 위에 있어도 20m 아래 풀밭을 눕혔다. 수직 거리 게이트를 넣었다.

두 번째 검수(야자수 흔들림 · 스넬의 창 · 지면 젖음)에서 또 셋이 걸렸다.

- **젖음 명부의 dedup 집합이 새고 있었다.** 전체 적용 루프가 죽은 머티리얼을 `entries`에서만
  지우고 `registered`에서는 안 지워, 그 엔트리가 프루닝의 시야에서도 사라져 집합만 계속 자란다.
- **야자수 명부 등록이 O(n²)였다.** 정글 특대 섬 하나에 야자수가 130그루쯤이고 섬이 50개면
  수천 그루인데, 그루마다 "지금까지 만든 전부"를 선형 스캔했다. 섬 생성이 눈에 띄게 걸릴 값이다.
- **흔들림 축이 "섬 뿌리 회전 = identity"라는 암묵 가정에 기대고 있었다.** 지금은 사실이지만
  나중에 섬을 회전 배치하면 조용히 어긋난다. 축을 부모 공간으로 옮겨 가정 자체를 없앴다.

또 하나: `GlossyEnvironmentReflection`은 URP 버전·Forward+ 여부에 따라 인자 개수가 다른 오버로드가
있어, 컴파일러 없이 작업하는 상황에서 감수할 이유가 없는 위험이었다. 큐브맵을 직접 샘플하는
동등한 코드로 바꿨다.
