...
## 에이전트 비용 규율 (0.2.51 추가 — 모든 작업 에이전트 지시문에 적용)

1. **과업 분할**: 에이전트 하나는 30~40턴 안에 끝나는 크기로 잘라 투입한다.
   파일 5개 이상을 고치는 과업은 쪼개서 순차 투입한다.
2. **API_REFERENCE 우선**: 외부 API 확인은 `Docs/API_REFERENCE.md`를 먼저 읽는다.
   원본 파일 전체 읽기 금지 — 거기 없는 심볼만 원본에서 필요한 선언만 grep으로 확인하고,
   쓰게 된 심볼은 API_REFERENCE.md에 추가한다.
3. **검증 1회**: `mcs --parse` 전체 실행은 파일마다가 아니라 **자기 과업 종료 시 1회**만.
   전수 적대적 검증은 디렉터가 별도 웨이브로 돌린다.
4. **보고는 결론만**: 과정 서술·표 남발 금지. 지시문이 요구한 항목만 보고한다.
