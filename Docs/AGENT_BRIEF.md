...
**"추가는 반드시 맨 끝에."** 씬·세이브가 정수/순서로 들고 있는 목록·enum은 중간 삽입·순서 변경·개명 시
기존 월드와 세이브가 조용히 깨진다. 해당 대상(상세는 7절 각 표):
`resourceEntries` · `hazardEntries` · `HazardType` · `BuildPieceType` · `EndingKind` · `FarmCropKind` ·
`MinimapUI.IslandMark`. `SaveData` 필드도 추가는 안전하지만 제거·개명은 파괴적이다.
