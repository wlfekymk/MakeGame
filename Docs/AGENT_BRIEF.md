...
> 갱신: **v0.2.00**  (예전 표기 0.01.128까지가 이 문서의 7절 씬 실측값 기준이다) · **커밋할 때마다 디렉터가 갱신한다.**
...
- **★ 가장 자주 필요한 것** — 키 배정 전수 표 · 비용 규율 요지 · Unity 함정 요지
- **1. 에이전트 비용 규율** (0.2.52 개정 — 전 에이전트 적용)
- **2. Unity·씬 함정** — 2.1 씬 값이 이긴다(사고 원인) · 2.2 알려진 함정(Unity 6.5 API 등)
- **3. rng·세이브 규율** — enum/순서 규칙 · rng 스트림 · 3.1 세이브
- **4. 키 배정** — 0.2.2x~0.2.50 신규 키 상세 (전수 표는 맨 앞 요약)
- **5. 설치형 시설** — 키트 8종 · 세이브 경로
- **6. 워크플로** — 6.1 현재 방향 · 6.2 절대 금지 · 6.3 보고서 형식
- **7. 기타 실측 자료** — 7.1 프로젝트 형태 · 7.2 파일 분할 지도 · 7.3 씬 실측값 · 7.4 아트 기준

---
## ★ 가장 자주 필요한 것

**키 배정 (전수)** — 새 키를 만들기 전에 반드시 확인 (신규 키 상세 → 4절):
...
`X` 회피 구르기 · `T`/마우스 우클릭 창 투척 · `Z` 낚시 · `F10` 전체 지도 · `F11` 치트 일괄 토글.

**비용 규율 요지** (전문 → 1절): Bash는 답만 출력(`tail`/`wc -l`/`grep -c`) · 파일 통째 Read 금지
(`sed -n`/`grep -A` 구간만) · 독립 호출은 한 턴에 병렬 · 기계적 변환은 스크립트 1회(치환 건수 assert) ·
검증은 `Tools/check.sh` 딱 1회.

**Unity 함정 요지** (전문 → 2절): 씬/프리팹 직렬화 값이 코드 기본값을 이긴다(씬 텍스트에 키가 없어도!) ·
`GetInstanceID()` = CS0619 에러 · `FindObjectsByType`은 1인자 형태만 · `OnGUI` 추가 금지 ·
timeScale 0 연출은 `unscaled*`만 · 주석이 "고쳤다"고 주장하면 값을 직접 확인.

---
## 1. 에이전트 비용 규율 (0.2.52 개정 — 모든 작업 에이전트 지시문에 적용)
...
### 2.1 이 프로젝트에서 사고가 나는 단 하나의 이유 — 씬 값이 이긴다
...
| `slotCapacity` | 씬 텍스트에 키가 없어 "씬이 안 덮어쓴다"고 판단 → 실제로는 인스펙터에 30이 박혀 있었다 (2절 함정 4번) |

**규칙: 수치를 논할 때는 (1) 코드 기본값 (2) 씬 직렬화 값 (3) 프리팹 오버라이드 세 곳을 모두 확인한다.**
**규칙: 주석이 "고쳤다"고 주장하면 값을 직접 확인한다. 이 프로젝트의 주석은 여러 번 거짓말했다.**
**규칙: "씬 텍스트에 키가 없다"는 "씬이 안 덮어쓴다"가 아니다. 2절 함정 4번을 반드시 읽어라.**
...
> ### ★ 2026-08-19 디렉터 결정 — **기존 세이브 호환은 더 이상 지키지 않는다**
> "세이브 데이터는 언제든 날려도 된다"는 지시가 내려왔다. 아직 출시 전이고 플레이어가 디렉터 한 명뿐이라,
> 옛 세이브를 지키려고 구조를 비틀던 비용이 이득보다 크다고 판단한 것이다.
>
> **그래서 이제 해도 되는 것**: `SaveData` 필드 제거·개명·순서 변경 / enum 값 중간 삽입·재번호·삭제 /
> `resourceEntries`·`hazardEntries` 같은 순서 의존 목록 재정렬 / `saveContentVersion` 자유롭게 올리기 /
> 마이그레이션 코드를 유지하지 말고 **지우기**.
> 구조가 더 깔끔해지는 쪽이 있으면 세이브 호환을 이유로 참지 마라.
>
> **그래도 지켜야 하는 것 (성격이 다르다)**:
> - **세이브가 깨져도 게임이 죽으면 안 된다.** 옛 파일을 읽다 예외가 나면 조용히 새 게임으로 떨어져야지,
>   널 참조로 터지면 안 된다. `JsonUtility`가 없는 필드를 기본값으로 채우는 성질에 계속 기댄다.
> - **`.bak`/`.tmp` 복구 경로는 그대로 둔다** (3.1). 이건 옛 세이브 호환이 아니라 **이번 판 저장이
>   중간에 끊겼을 때** 살리는 장치다. 성격이 완전히 다르다.
> - **rng 규율은 그대로다** (아래). 세이브와 무관하게 같은 worldSeed가 같은 월드를 만들어야
>   버그 재현이 된다.
> - 구조를 깨는 변경을 했으면 **보고에 한 줄로 알려라** — 디렉터가 새 게임으로 시작해야 하는지 알아야 한다.

**순서 의존 목록**(참고용으로 남긴다 — 이제 강제 규칙이 아니라 "여기를 건드리면 옛 세이브가 안 열린다"는
지도다): `resourceEntries` · `hazardEntries` · `HazardType` · `BuildPieceType` · `EndingKind` ·
`FarmCropKind` · `MinimapUI.IslandMark`.

**rng**: 월드 생성·배치는 `UnityEngine.Random` 금지, 섬별 시드 `System.Random` 스트림 사용
(→ 6절 절대 금지 6번). 새 배치 시스템은 전용 스트림(salt — 예: 난파선 `0xB0A7`)을 쓰고,
보장 배치는 자리당 rng 소비 횟수를 유지해 같은 worldSeed의 기존 월드를 밀지 않는다(7절 위험 요소).
...
`JsonUtility` 는 없는 필드를 기본값으로 채운다. (제거·개명이 옛 세이브를 깨는 것은 맞지만,
2026-08-19 결정으로 **그게 더 이상 막는 이유가 되지 않는다** — 위 상자 참고.)
...
전수 한 줄 표는 맨 앞 "★ 가장 자주 필요한 것"에 있다. 아래는 0.2.2x~0.2.50 신규 키의 상세다.
...
**새 키를 만들기 전에 맨 앞 요약의 전수 표를 확인해라.** 남는 키가 거의 없다.
→ 위 세 키(X·T·Z)는 전부 **씬이 아니라 코드 기본값**이다. 씬에 직렬화되지 않았으므로 2절의
"씬 값이 이긴다"가 적용되지 않는다 — 코드를 고치면 그대로 반영된다.

---
## 5. 설치형 시설 (설치 키트, `ItemData.isPlaceable`)
...
### 6.1 ★ 현재 방향 (2026-08-16 전환 — 이전 문서들과 다르다)
...
1. **소유 파일 밖 편집 금지.** 매 웨이브 3~4명이 병렬 편집한다. 남의 파일을 건드리면 덮어쓰기 사고다.
   → 단, **작업을 끝내려면 필요한데 금지된 파일이 있으면 조용히 우회하지 말고 보고해라**(2절 함정 5번).
...
7. **`Cursor.lockState` 를 직접 만지지 마라.** `CursorLockController` 가 단독으로 결정한다(7절 씬 실측값).
...
[막힘] <파일>: <왜 필요한가>   ← 작업 완료에 필요한데 락에 걸린 파일 (필수 · 2절 함정 5번)
```
...
### 7.2 파일 분할 지도 (0.2.03~0.2.06 리팩토링)
...
### 7.3 씬 실측값 (v0.01.128 = 0.2.00 직전 기준)
...
**프리팹 값이 이긴다.** (2절의 WaterStill 사고가 정확히 이것이다. 현재 프리팹 = `waterPerSecond 0.1`
...
span **23.2 × 23.4 km** · 최근접 **1,095 m** · 이웃 섬까지 중앙값 **1,458 m** ·
시작 섬 = **남서쪽 구석 (-10400, -10400)** · **시작 섬 반경 4 km 안은 소형 7개뿐** ·
시작 → 대형 3개 **10.4 / 16.7 / 22.5 km** · 시작 → **특대 29.8 km** · 좌표 절댓값 최대 11.7 km(< 반폭 20 km).
**특대 섬은 군집에 속하지 않는 외딴섬**이다 — 대각선 반대편 구석에 혼자 서 있고 가장 가까운
이웃까지 **7.4 km**다(`XL_SOLITUDE`가 강제한다). 특대를 뺀 나머지 중 가장 먼 섬은 25.3 km.
2026-08-20 재조정: 가까운 간격 +32%(최소 830→1,095), 먼 거리 −19%(31.2→25.3 km) —
"붙어 있던 것은 벌리고 멀리 있던 것은 당긴다"는 디렉터 지시.
...
(`IslandTravel.CurrentBypass.OceanReadyWithMotor`). 29.8 km를 노 1.2 / 돛 4.0 / 모터 6.0 m/s로 나누면
6시간 54분 / 2시간 4분 / 1시간 23분이라, 모터가 유일하게 현실적인 수단이다.
(지도 클릭 이동은 여전히 즉시다 — 이 거리는 직접 몰고 갈 때의 이야기다.)
...
> 이 절에서 빠진 하위 절: **키 배정 → 4절(전수 표는 맨 앞 요약)** · **설치형 시설 → 5절** · **세이브 → 3절.**


#### 엔딩
배(귀환): 뗏목 `RaftStructure.IsOceanReady`(바닥판 6칸 이상 + 돛+키 또는 모터) +
비상식량 12 · 생수 12 · 연료 1 + **경과 15일**(씬). 구 3단계 도면(`BoatConstructionSystem`)은 삭제됐다 —
뗏목은 해안에 직접 짓는 자유 건조다(`EndingChecker.cs:409` 재배선 주석).
...
> **"config만 고치면 반영된다"고 가정하지 마라.** 이 프로젝트 사고의 대표 유형이다(2절 표 참고).
...
> ⚠️ 소스 주석에 낡은 아이템 수(31/32/51)가 남아 있었으나 **0.2.52에서 전부 57로 정정됐다.**
> 그래도 **수를 셀 때는 주석이 아니라 레지스트리 에셋을 세라**(2절 "주석이 고쳤다고 주장하면
> 값을 직접 확인한다"의 전형이다).
