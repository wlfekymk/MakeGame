// MGGrass - Graphics.RenderMeshInstanced로 그리는 인스턴싱 잔디 전용 URP 셰이더.
//
// GrassFieldSystem이 Resources.Load<Shader>("Shaders/MGGrass")로 로드해 잔디 머티리얼에만 쓰고,
// 로드가 실패하면 시스템 쪽에서 URP Lit 경로로 폴백한다(게임은 이 셰이더 없이도 돌아가야 한다 -
// MGOcean/MGKelpSway와 같은 폴백 계약).
//
// 설계 계약(GrassFieldSystem 쪽과 맞물린다):
//  * blade 메시: 교차 쿼드 2장(가로 ~0.14m, 세로 ~1m), 피벗 밑동, UV.y = 0(뿌리)~1(끝).
//    높이 변주는 인스턴스 행렬 스케일로 들어온다 - 셰이더는 UV.y 가중만 쓰므로 스케일과 무관하게
//    "뿌리 고정, 끝이 크게" 굽는 형태가 유지된다(MGKelpSway의 y² 가중과 같은 발상, 기준만 UV).
...
//    퍼 인스턴스 커스텀 데이터는 받지 않는다 - 색 지터도 인스턴스 행렬의 월드 원점 해시로만 만든다.
//  * 정점 바람: UV.y² 가중(뿌리 고정) × 월드 XZ 기반 스크롤 사인 3겹(파장 18m + 4m + 교차 플러터).
//    큰 파장이 "바람이 지나가는 물결"을 만들고 작은 파장이 잘게 떤다. 위상은 blade 원점(피벗)
//    기준 - 한 포기는 통째로 굽는다(MGKelpSway의 찢김 방지 규칙. 폭 0.14m라 어차피 위상차도 미미).
//    진폭 끝 기준 합계 ~0.12m × _WindStrength. 콜라이더 없는 순수 장식이라 판정과 어긋나지 않는다.
//  * 밟힘: blade 원점과 _MG_PlayerPos의 수평 거리 < _TrampleRadius면 끝을 플레이어 반대 방향으로
//    밀고 눕힌다(부드러운 falloff, UV.y 가중). 눌린 만큼 바람은 죽인다(누운 풀이 또 흔들리면 이상).
...
//  * 색: UV.y로 _RootColor→_TipColor 세로 그라데이션 + 원점 해시 지터(_DryTint 0~35% 혼합,
//    명도 ±8%). 정점에서 지터 적용한 두 색을 만들어 보간으로 넘긴다 - 프래그먼트는 lerp 한 번.
//  * 라이팅: 메인 라이트 램버트 + SH 앰비언트 + URP 포그의 Opaque(좁은 쿼드라 컷아웃 불요).
//    Cull Off 양면 - 뒷면은 SV_IsFrontFace로 노멀을 뒤집고, 잔디 노멀은 위쪽으로 절반 굽혀
//    (lerp(faceNormal, up, 0.6)) 카드 특유의 명암 끊김을 부드럽게 편다.
//  * ShadowCaster/DepthOnly 패스는 넣지 않는다 - 수만 포기 그림자 캐스팅은 비용만 크고,
//    패스가 없어도 렌더는 망가지지 않는다(MGOcean의 "패스 생략 - 우아한 열화"와 같은 선택).
//  * CBUFFER(UnityPerMaterial)에 Properties 스칼라/색/벡터 전부 - SRP Batcher 호환.
Shader "MG/Grass"
{
    Properties
    {
        _RootColor("뿌리 색(어두운 초록)", Color) = (0.09, 0.22, 0.06, 1)
        _TipColor("끝 색(밝은 초록)", Color) = (0.38, 0.62, 0.18, 1)
        _DryTint("마른 풀 색(지터 혼합 목표)", Color) = (0.55, 0.52, 0.26, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 얇은 blade 카드의 뒷면도 그린다(잔디밭은 어느 방향에서든 보인다).
            Cull Off
            ...
            // 3겹: 1겹과 직교 방향의 약한 플러터 - 한 방향 사인만 있을 때의 "줄 맞춰 흔들림"을 부순다.
            ...
            // 원점 XZ → 0~1 해시. 퍼 인스턴스 데이터 없이 색 지터를 만드는 표준 정점 해시.
            ...
            struct Varyings
            {
                ...
                // 지터(마른 풀 혼합 + 명도) 적용이 끝난 뿌리/끝 색 - 프래그먼트는 uv.y로 lerp만 한다.
            ...
            Varyings vert(Attributes IN)
            {
                ...
                // blade 원점(밑동) 월드 위치 - 모델 계약상 피벗이 밑동이다. 위상/밟힘/지터의 기준.
                ...
                // ---- 밟힘: 원점-플레이어 수평 거리 기반. blade 단위로 통째로 눕는다. ----
                ...
                // ---- 원점 해시 색 지터: _DryTint 0~35% 혼합 + 명도 ±8%. ----
            ...
            half4 frag(Varyings IN, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                // 세로 그라데이션: 지터 적용이 끝난 두 색을 UV.y로 보간.
                half3 albedo = lerp(IN.rootCol, IN.tipCol, saturate(IN.uv.y));
