...
def enforce_contract(obj, tri_budget, expect_size=None, size_tol=0.005,
                     tri_floor=None, name=None):
    """계약을 **적용하고 검증한다**. 하나라도 어기면 ContractError.

    적용:
      (a) 밑면을 y = 0 으로 내린다
      (b) X / Z 를 바운딩 박스 중심으로 정렬한다
      (c) 오브젝트 변환을 정점에 굽는다
    검증:
      (1) 모든 면이 삼각형인가
    ...
    # (a)(b) 원점 정렬.
    lo, hi = bbox(obj)
    offset = Vector((-(lo.x + hi.x) * 0.5, -lo.y, -(lo.z + hi.z) * 0.5))
    ...
    if abs(lo.x + hi.x) > EPS or abs(lo.z + hi.z) > EPS:
        raise ContractError(
...
def verify_obj_file(path, stats, size_tol=0.005):
    ...
    if abs(min(xs) + max(xs)) > 1e-3 or abs(min(zs) + max(zs)) > 1e-3:
