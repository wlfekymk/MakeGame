...
HALF_EXTENT   = 14000.0   # 월드 상자 반폭. 좌표는 ±14 km에 들어간다
MIN_SPACING   = 780.0     # 같은 군집 안에서 두 섬 중심 사이 최소 거리(예전 340 m × 2.3)
...
XL_TARGET_DIST   = 19000.0   # 특대 섬을 놓고 싶은 거리(모터 6 m/s로 약 53분 = 5.3일)
...
def assign_sizes(pts):
    ...
    # 특대: 목표 거리에 가장 가까운 섬 하나
    xl = min(others, key=lambda t: abs(t[1] - XL_TARGET_DIST))
    sizes[xl[0]] = "ExtraLarge"

    # 대형 3: LARGE_MIN_DIST 밖에서 거리 구간을 3등분해 하나씩
    pool = [t for t in others if t[1] >= LARGE_MIN_DIST and t[0] not in sizes]
    far = max(t[1] for t in others)
