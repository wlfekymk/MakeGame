...
| v0.01.070 | **사용자 추가 피드백 2건 반영**: (1) "같은 종류 자원이라도 보여지는 모양이 다양했으면 좋겠다" - `IslandResourceSpawner.SpawnSingleNode`가 스폰마다 축별로 다른 크기 배율(0.85~1.25배)과 Y축 무작위 회전을 곱하도록 수정하고, `AddResourceDetailParts`의 나뭇가지 곁가지(1~3개)·대나무 마디(2~4개)·돌조각 곁돌(0~3개)·코코넛 추가 열매(0~2개)·천조각 주름(70% 확률)·야자잎 개수(4~6장)+펼침각·금속조각 꺾임각·엔진부품 볼트(3~6개)를 전부 무작위 범위로 바꿔, 같은 자원 타입이라도 인스턴스마다 조금씩 다르게 보이도록 함. (2) "섬 크기가 지금의 10배는 커져야 할 것 같다" - `WorldMapManager.GetSizeScale`의 4단계 반지름을 5/9/14/20 → 50/90/140/200으로 비례 확대(Small<Medium<Large<ExtraLarge 순서 유지)하고, 연쇄적으로 영향받는 값들을 함께 조정: 섬 배치 간격(`baseDistanceStep` 120→1200, `distanceJitter` 40→400, `minSpacingBetweenIslands` 60→500), 바다 크기(`oceanSize` 4000→40000), 지형 메시 해상도(반지름 비례 `ringCount`/`radialSegments` 동적 계산으로 큰 섬도 각지지 않게), 모래 텍스처 타일링(반지름 비례), 자원/위험요소/사냥감 산포 반경(`scatterRadius` 8/10/9 → 80/100/90), 상어 배치 안전거리(`minDistanceFromIslands` 35→350, `safeZoneRadiusFromStart` 60→600), 미니맵 표시 반경(`radarWorldRadius` 400→4000). 씬 파일(`SampleScene.unity`)에 저장된 override 값도 스크립트 기본값과 함께 갱신. Play 모드에서 섬 배치 거리 확대(Inspector상 Map Position 예: (-720, 0, 515))와 나뭇가지 노드 2개가 곁가지 개수/각도/스케일이 서로 다르게 나오는 것을 Scene 뷰로 라이브 확인 |

| v0.01.071 | **1000개 작업 목록 처리 재개(#327~332)**: "일반 재료" 대분류(나뭇가지~엔진부품 12종)가 무기/의료/음식/음료/설치형/이동수단 어디에도 안 걸려 전부 갈색 한 가지로만 표시되던 문제를 발견해 `UIBuilder.GetMaterialSubCategoryColor` 신규 메서드로 목재/석재/금속/식물/열매/보급품 6계열 색조 세분화(인벤토리·제작 UI·월드 자원 노드 3곳에 동시 반영). 야자잎 드로우콜(성능 배치로 이관)·소형 자원 채집 판정 범위(고정 4m, 버그 없음)·키 큰 자원 시야 방해(의도된 지형지물, 수정 없음)를 코드로 검토해 조치/보류 결정. Assets Refresh 재컴파일 콘솔 에러 0건, Play 모드에서 Resource_돌조각 인스펙터로 새 회색 재질이 실제로 반영됨을 라이브 확인 |
| v0.01.072 | **멀티 에이전트 개발 체제 전환 + 배치 1(10건) 처리**: 디렉터 1 + 역할 에이전트 7(game-designer/systems-engineer/ui-engineer/tech-artist/qa-reviewer/unity-operator/build-master) 구조 도입, `.claude/agents/*.md`·`Docs/MULTI_AGENT_GUIDE.md`·CLAUDE.md 라우팅 규칙 추가. 파일 소유권=락 원칙, Unity 제어는 unity-operator 1개만 단독 실행. **치명 결함 수정**: `IslandGenerator.GenerateNextIslandSize`가 spawnConfig를 null 체크 없이 참조해 미연결 시 WorldMapManager.Start 체인에서 NRE로 월드 생성이 전면 중단되던 문제에 null 가드+LogError 폴백 추가. **신규 `Utils/IslandSizeMetrics.cs`**: 3개 스포너에 중복돼 있던 섬 규모별 배율/반경을 단일 소스로 통합(단, 씬 직렬화 값이 항상 우선하고 미설정 시에만 폴백하는 방식이라 SampleScene의 기존 값 1/2/3/4·반경 80/100/90은 그대로 보존). **신규 `Systems/CreatureVisualBuilder.cs`**: 독사 혀·전갈 꼬리/집게·함정 가시·육상 사냥감 다리 4종 프리미티브 디테일 추가(위험요소와 사냥감 실루엣 혼동 해소). **성능**: GameOverController·EndingChecker·MainMenuController·SettingsMenuController의 OnGUI 매 프레임 `new GUIStyle` 제거(EnsureStyles 지연 캐싱), SurvivalHudUI·InventoryUI 텍스트를 값 변경 시에만 갱신하도록 캐싱. **정리**: InventoryUI.GetCategory와 UIBuilder.GetItemCategoryColor의 중복 분류 로직을 `UIBuilder.GetItemCategory`로 단일화, SaveLoadController.FindItemDataByName을 이름→ItemData 딕셔너리 캐시로 교체, SurvivalStats에 위험 임계값 5종 public const 노출. **오디오**: 파도 앰비언트 4초→18초·비 4초→12초로 확장하고 파도에 27초 주기 저주파를 섞어 반복감 완화, PlayCraftSuccess 1종 재사용이던 성공음을 PlayHealSuccess/PlaySleepSuccess/PlayCampfireLit 3종으로 분화. Assets>Refresh 재컴파일 콘솔 에러 0건(Assembly-CSharp.dll 재빌드 확인), Play 스모크 테스트에서 바다·섬·자원·생물·타이틀 UI 정상 생성 및 런타임 예외 0건을 라이브 확인 |
